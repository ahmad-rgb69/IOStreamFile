...
public class BukuKontak {
    ...
    public void cariKontak(String nama){ //Soal Latihan Mandiri no.1
        for (Kontak k : daftar) {
    ...
    public void hapusKontak(String nama){ //Soal Latihan Mandiri no.3
       for (int i=0; i < daftar.size(); i++){
           Kontak k = daftar.get(i);
           if (k.getNama().equalsIgnoreCase(nama)){
               daftar.remove(i);
    ...
    public void muatDariBerkas(){
        ...
        try(BufferedReader pembaca = new BufferedReader(new FileReader(namaBerkas))){
            ...
            while((baris = pembaca.readLine()) != null) {
                ...
                if(bagian.length == 2) {
                    daftar.add(new Kontak(bagian[0], bagian[1], bagian[2] /*Soal Latihan Mandiri no.2*/));
                }
            }
